...
class _MyBottomNavigationBarState extends State<MyBottomNavigationBar> with SingleTickerProviderStateMixin {
  ...
  Widget getItem(int index) {
    final MyBottomNavigationBarItem item = itemList![index];
    ...
    if (MyBottomNavigationBarType == MyBottomNavigationBarType.normal) {
      return Container(
        child: (_currentIndex == index)
            ? Wrap(
                ...
                children: <Widget>[
                  item.activeIcon ?? Icon(item.activeIconData, size: activeIconSize ?? item.activeIconSize ?? 14, color: activeIconColor ?? item.activeIconColor ?? theme.primaryColor),
                  widget.labelDirection == Axis.horizontal ? MySpacing.width(showActiveLabel ? widget.labelSpacing : 0) : MySpacing.height(showActiveLabel ? widget.labelSpacing : 0),
                  showActiveLabel
                      ? Text(
                          item.title!,
                          style: activeTitleStyle ?? item.activeTitleStyle ?? MyTextStyle.labelSmall(color: activeTitleColor ?? item.activeTitleColor ?? theme.primaryColor, fontSize: activeTitleSize ?? item.activeTitleSize),
                        )
                      : Container(),
                ],
              )
            : Wrap(
                ...
                crossAxisAlignment: WrapCrossAlignment.center,
                children: <Widget>[
                  item.icon ?? Icon(item.iconData, size: iconSize ?? item.iconSize ?? 14, color: iconColor ?? item.iconColor ?? theme.colorScheme.onSurface),
                  widget.labelDirection == Axis.horizontal ? MySpacing.width(showLabel ? widget.labelSpacing : 0) : MySpacing.height(showLabel ? widget.labelSpacing : 0),
                  showLabel
                      ? Text(
                          item.title!,
                          style: titleStyle ?? item.titleStyle ?? MyTextStyle.labelSmall(color: titleColor ?? item.titleColor ?? theme.colorScheme.onSurface, fontSize: widget.titleSize ?? item.titleSize),
                        )
                      : Container(),
                ],
              ),
      ...
      if (item.activeIcon != null) {
        ...
        iconWidget = Icon(item.activeIconData ?? item.iconData, size: activeIconSize ?? item.activeIconSize ?? 24, color: activeIconColor ?? item.activeIconColor ?? theme.primaryColor);
      }
      ...
      return (_currentIndex == index)
          ...
          : item.icon ?? Icon(item.iconData, size: iconSize ?? item.iconSize ?? 24, color: iconColor ?? item.iconColor ?? theme.colorScheme.onSurface.withAlpha(150));
    }
  }
  ...
  List<Widget> getListOfItems() {
    ...
    final double singleWidth = (MediaQuery.of(context).size.width - 50) / (itemList!.length + (widget.showLabel ? 0 : (showActiveLabel ? 0.5 : 0)));

    for (int i = 0; i < itemList!.length; i++) {
      final double containerWidth = widget.showLabel ? (singleWidth) : (showActiveLabel ? (i == _currentIndex ? singleWidth * 1.5 : singleWidth) : singleWidth);
      list.add(
        SizedBox(
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: <Widget>[
        Expanded(
          child: TabBarView(physics: const ClampingScrollPhysics(), controller: _tabController, children: getListOfViews()),
        ),
        Container(
