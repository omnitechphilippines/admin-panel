...
import '../widgets/my_list_extension.dart';
import '../widgets/my_responsiv.dart';
...
class MyFlex extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MyResponsive(
      builder: (BuildContext context, BoxConstraints constraints, MyScreenMediaType screenMediaType) {
        double width = constraints.maxWidth;
        List<Widget> list = [];
        for (List<MyFlexItem> lCol in getGrouped(screenMediaType)) {
          list.addAll(
            lCol.mapIndexed((index, col) => Container(width: getWidthFromFlex(width, col.flex[screenMediaType] ?? MyScreenMedia.flexColumns.toDouble(), lCol.length, spacing ?? flexSpacing), padding: getPadding(index, lCol.length), child: col)),
          );
        }
        return Wrap(crossAxisAlignment: wrapCrossAlignment, alignment: wrapAlignment, runAlignment: runAlignment, runSpacing: (runSpacing ?? flexSpacing), spacing: (spacing ?? 0), children: list);
      },
    );
  }
